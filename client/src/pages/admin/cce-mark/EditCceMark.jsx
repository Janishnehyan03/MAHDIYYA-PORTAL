...
function EditCceMark() {
  ...
  const fetchResults = useCallback(async () => {
    ...
    try {
      const response = await Axios.get(
        `/cce/fetch?examId=${exam}&subjectId=${subject}&classId=${selectedClass}`
      );
  ...
  const handleMarkChange = (index, newMarks) => {
    setUpdatedData((prevData) => {
      ...
      newData[index] = { ...newData[index], cceMark: newMarks }; // Update cceMark for the corresponding item
      return newData;
    });
  };
  ...
  const handleSubmit = async () => {
    try {
      const response = await Axios.patch(
        `/cce?examId=${exam}&subjectId=${subject}&classId=${selectedClass}`,
        updatedData
      );
    ...
    <div>
      ...
      <div className="bg-gray-50 flex-col flex items-center justify-center">
        ...
        <div className="w-1/2 mx-auto flex-col items-center justify-center flex">
          <h1 className="px-3 text-sky-500 bg-gray-100 text-center">
          ...
          <button
            ...
            className="lg:w-1/2 w-full mx-auto  bg-sky-500 text-white p-3 font-bold uppercase my-5 hover:bg-sky-400"
          >
            Edit Results{" "}
          </button>
        </div>
